...
import { fetchHomeContent, updateHomeContent, setActiveTab } from '../store/slices/adminSlice';
import { GlassCard } from '../components/GlassCard';
import { Save, FileText, Globe, Trash2 } from 'lucide-react';
import api from '../services/api';

export default function CMS() {
  const dispatch = useDispatch();
  const homeContent = useSelector((state) => state.admin.homeContent);
  ...
  const [microLines, setMicroLines] = useState([]);
  const [newMicroMsg, setNewMicroMsg] = useState('');

  // Legal documents states
  const [selectedDocType, setSelectedDocType] = useState('privacy_policy');
  ...
  useEffect(() => {
    dispatch(setActiveTab('cms'));
    loadLandingData();
  }, [dispatch]);

  const loadLandingData = async () => {
    setLoading(true);
    const result = await dispatch(fetchHomeContent()).unwrap();
    if (result) {
      setHeroHeadline(result.hero?.headline || '');
      setHeroSubtext(result.hero?.subtext || '');
      setHeroCta(result.hero?.cta || '');
      setAnticipipation(result.anticipation || '');
      setLastSignal(result.lastSignal || '');
      setTrust(result.trust || '');
      setLocked(result.locked || '');
      setUpgradeHeadline(result.upgrade?.headline || '');
      setUpgradeSubtext(result.upgrade?.subtext || '');
      setUpgradeCta(result.upgrade?.cta || '');
      setWaitingMessages(result.waitingState || []);
      setGuidanceList(result.guidance || []);
      setMicroLines(result.microLines || []);
    }
    setLoading(false);
  };
  ...
  const loadLegalDoc = async (type) => {
    ...
    try {
      ...
      setDocTitle(type.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '));
      setDocContent('');
    }
    setLoading(false);
  };
  ...
  const handleSaveLanding = async (e) => {
    ...
    await dispatch(updateHomeContent(payload)).unwrap();
    setLoading(false);
  ...
  const handleSaveLegal = async (e) => {
    ...
    setLoading(true);
    try {
      await api.put(`/system/legal/${selectedDocType}`, { title: docTitle, content: docContent });
      alert('Legal document updated and synchronized successfully! 📜');
    } catch (err) {
      console.warn(`Failed to update legal doc ${selectedDocType}`, err);
      alert('Failed to save document. Operational fallback enabled.');
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-3 space-y-2">
                ...
                {['privacy_policy', 'terms_and_conditions', 'about_us', 'faq', 'support'].map((type) => (
                  <button
                    key={type}
                    type="button"
