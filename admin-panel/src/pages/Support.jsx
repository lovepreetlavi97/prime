...
import { Search, CheckCircle2, Reply, HelpCircle, User, Mail, Phone, Calendar } from 'lucide-react';
...
export default function Support() {
  ...
  const loadSubmissions = async () => {
    ...
    try {
      ...
      if (data && data.success) {
        ...
        if (selectedSub) {
          const found = data.data.find((s) => s._id === selectedSub._id);
      ...
      console.warn('Failed to fetch contact submissions, falling back to mock data.', e);
  ...
  const handleResolve = async (e) => {
    ...
    setResolving(true);
    try {
      await api.put(`/system/contact/${selectedSub._id}/resolve`, { replyText });
      setReplyText('');
      alert('User inquiry resolved and response logged!');
      loadSubmissions();
    } catch (err) {
      console.warn(`Failed to resolve submission ${selectedSub._id}`, err);
      // Fallback local modification
      const updated = { ...selectedSub, status: 'RESOLVED', reply: replyText };
      setSelectedSub(updated);
      setSubmissions(prev => prev.map(s => s._id === selectedSub._id ? updated : s));
      setReplyText('');
      alert('User inquiry resolved locally (offline fallback mode)!');
    } finally {
      setResolving(false);
    }
  };
