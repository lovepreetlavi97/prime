...
export default function Brokers() {
  ...
  const brokerConnections = useSelector((state) => state.admin.brokerConnections);
  const [logs, setLogs] = useState([]);
  const [syncing, setSyncing] = useState(false);

  useEffect(() => {
    dispatch(setActiveTab('brokers'));
  }, [dispatch]);

  useEffect(() => {
    // Generate real-time API logs
    const timer = setInterval(() => {
      const brokersList = ['Zerodha (Kite)', 'Groww', 'Upstox', 'AngelOne'];
      const actions = ['verified TOTP token', 'fetched holdings margin', 'placed CE buy block order', 'heartbeat refresh completed'];
      const randomBroker = brokersList[Math.floor(Math.random() * brokersList.length)];
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <button
              ...
              disabled={syncing}
              className="h-9 px-4 rounded-xl bg-amber-500/10 border border-amber-500/20 hover:border-amber-500/40 text-[#D4AF37] font-black text-[9px] tracking-[2px] uppercase flex items-center gap-1.5 transition-all duration-300 cursor-pointer"
            >
