...
export default function UsersPage() {
  const { 
    users, fetchUsers, toggleBanUser, deleteUser, 
  ...
  const [overridePlan, setOverridePlan] = useState<'free' | 'pro' | 'elite'>('pro');
  const [overrideDuration, setOverrideDuration] = useState('30');
  const [overrideRole, setOverrideRole] = useState<'USER' | 'ADMIN' | 'ANALYST' | 'SUPPORT'>('USER');
  ...
  const handleApplySubscription = async (userId: string) => {
    const days = parseInt(overrideDuration) || 30;
    await updateUserSubscription(userId, overridePlan, days);
    // Refresh selections
    const updated = users.find(u => u._id === userId);
    if (updated) setSelectedUser({ ...updated, subscription: { plan: overridePlan, isActive: true, endDate: new Date(Date.now() + 86400000 * days).toISOString() } });
    alert('Subscription package successfully manually assigned!');
  };

  const handleApplyRole = async (userId: string) => {
    await updateUserRole(userId, overrideRole as any);
  ...
  return (
    <div className="space-y-6 select-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    filteredUsers.map((user) => (
                      ...
                      >
                        ...
                        <td className="p-4 text-right pr-6 space-x-2">
                          <button
                            onClick={(e) => { e.stopPropagation(); toggleBanUser(user._id); }}
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); if (confirm('Delete user permanently?')) deleteUser(user._id); }}
        ...
        {/* Right column: Selected User Detailed Analytics Overrides */}
